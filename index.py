...
#		r_HOD = hodclient.post_request(files = {'file': open('./image_temp.png', 'rb')}, 'https://api.havenondemand.com/1/api/sync/detectfaces/v1', False)
		r_HOD = hodpostrequests('detectfaces', files={'file': open('./image_temp.png', 'rb')})
		print "------------"
		print "POST to HOD"
		for face in r_HOD['face']:
			if len(face) > 0: # is there a face in the image
				...
				url = 'http://fd-camera.helionit.io/image_processor'
				proxyDict = {}
				data = {}
				...
				del r_server
				##
				os.system('sudo fbi -T 2 -d /dev/fb0 -noverbose -a ./img/' + img_name + str(img_counter) + '.png') #display image
...
def slide_show():
	while True:
		...
		for i in range(0, img_counter+1):
			...
			os.system('sudo fbi -T 2 -d /dev/fb0 -noverbose -a ./img/' + img_name + str(i) + '.png')
